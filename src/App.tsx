...
import { Menu, Plus, Trash2, Home, PanelLeftClose, PanelLeftOpen, BookOpen, Edit3, Columns, Download, Upload, Moon, Sun, SquarePen, Search } from 'lucide-react';
    ...
    <div className="flex h-screen w-full overflow-hidden bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 font-sans transition-colors duration-200">
      ...
      {!isSidebarVisibleDesktop && (
        <div className="hidden md:flex flex-col items-center py-4 gap-6 w-16 border-r border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 h-full flex-shrink-0">
          <button onClick={() => setIsSidebarVisibleDesktop(true)} className="p-2.5 rounded-xl bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 transition-colors" title="Show Sidebar">
            <PanelLeftOpen size={20} />
          </button>
          
          <div className="flex flex-col items-center gap-4 mt-2">
            <button onClick={handleCreateNote} className="p-2 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors" title="New Note">
              <SquarePen size={20} />
            </button>
            <button onClick={() => setIsSidebarVisibleDesktop(true)} className="p-2 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors" title="Search">
              <Search size={20} />
            </button>
            <button onClick={() => handleSelectNote(null)} className="p-2 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors" title="Home">
              <Home size={20} />
            </button>
            <button onClick={toggleTheme} className="p-2 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors" title="Toggle Theme">
            ...
            <button 
              ...
              className={cn(
                ...
                activeNote ? "hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100" : "text-zinc-300 dark:text-zinc-700 cursor-not-allowed"
              )} 
              title={activeNote ? "Export Note" : "Open a note to export"}
            >
              <Download size={20} />
            </button>
            <button onClick={() => fileInputRef.current?.click()} className="p-2 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors" title="Import Notes">
              <Upload size={20} />
            </button>
          </div>
        </div>
              ...
              <div className="flex bg-zinc-200 dark:bg-zinc-800 rounded-lg p-1">
                <button 
                  className={cn(
                    ...
                    mobileTab === 'edit' ? "bg-white dark:bg-zinc-950 shadow-sm" : "text-zinc-500 dark:text-zinc-400"
                  )}
                ...
                <button 
                  className={cn(
                    "px-3 py-1 text-sm rounded-md transition-all", 
                    mobileTab === 'preview' ? "bg-white dark:bg-zinc-950 shadow-sm" : "text-zinc-500 dark:text-zinc-400"
                  )}
            ...
            <div className="flex items-center bg-zinc-100 dark:bg-zinc-900 rounded-lg p-1">
              <button 
                ...
                className={cn(
                  ...
                  desktopViewMode === 'edit' ? "bg-white dark:bg-zinc-800 shadow-sm text-zinc-900 dark:text-zinc-100" : "text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300"
                )}
              >
                <Edit3 size={14} /> Edit
              </button>
              <button 
                ...
                className={cn(
                  "px-3 py-1.5 text-sm rounded-md flex items-center gap-2 transition-all", 
                  desktopViewMode === 'both' ? "bg-white dark:bg-zinc-800 shadow-sm text-zinc-900 dark:text-zinc-100" : "text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300"
                )}
              >
              ...
              <button 
                ...
                className={cn(
                  "px-3 py-1.5 text-sm rounded-md flex items-center gap-2 transition-all", 
                  desktopViewMode === 'read' ? "bg-white dark:bg-zinc-800 shadow-sm text-zinc-900 dark:text-zinc-100" : "text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300"
                )}
              >
          ...
          {activeNote ? (
            <>
              <div className={cn(
                ...
                desktopViewMode === 'edit' || desktopViewMode === 'both' ? 'md:flex md:flex-1' : 'md:hidden'
              )}>
              ...
              <div className={cn(
                ...
                desktopViewMode === 'read' || desktopViewMode === 'both' ? 'md:flex md:flex-1' : 'md:hidden'
              )}>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-zinc-50 dark:bg-zinc-900/50">
              <div className="max-w-5xl mx-auto">
                <h1 className="text-3xl font-bold mb-8 hidden md:block">All Notes</h1>
                {notes.length === 0 ? (
                  <div className="text-center py-20 text-zinc-500">
                    <div className="text-4xl mb-4">📝</div>
                    <h2 className="text-xl font-medium mb-2">No Notes Yet</h2>
                    <p>Create your first note to get started.</p>
                    <p className="hidden md:block text-sm mt-4 opacity-70">Ctrl + N to create</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {notes.map(note => (
                      ...
                      >
                        <h3 className="font-semibold text-lg mb-2 truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{note.title || 'Untitled Note'}</h3>
